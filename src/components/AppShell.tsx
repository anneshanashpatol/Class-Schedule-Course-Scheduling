import { Bot, CalendarDays, ClipboardList, KeyRound, LogOut, Menu, Pencil, Users, X } from 'lucide-react';
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}><div className="sidebar__top"><div className="brand"><span className="brand__mark"><CalendarDays /></span><span>前程π日程</span></div><button className="icon-button mobile-only" aria-label="关闭菜单" onClick={() => setOpen(false)}><X /></button></div><nav aria-label="主导航">{links.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => isActive ? 'active' : ''}><Icon size={19} />{label}</NavLink>)}<a href="https://qcp.dpdns.org/assistant" target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}><Bot size={19} />排课 AI 助手</a></nav><div className="sidebar__user"><button className="avatar avatar-button" aria-label="查看我的资料" onClick={() => setProfileOpen(true)}>{user.displayName.slice(0, 1)}</button><button className="sidebar__identity" onClick={() => setProfileOpen(true)}><strong>{user.displayName}</strong><span>{roleNames[user.role]} · 我的资料</span></button><button className="icon-button" onClick={() => void logout()} aria-label="退出登录"><LogOut size={18} /></button></div></aside>
