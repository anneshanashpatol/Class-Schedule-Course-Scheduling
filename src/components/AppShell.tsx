...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <header className="mobile-header"><div className="brand"><span className="brand__mark"><CalendarDays /></span><span>青禾排课</span></div><button className="icon-button" aria-label="打开菜单" onClick={() => setOpen(true)}><Menu /></button></header>
    {open && <button className="sidebar-scrim" aria-label="关闭菜单" onClick={() => setOpen(false)} />}
    <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}><div className="sidebar__top"><div className="brand"><span className="brand__mark"><CalendarDays /></span><span>青禾排课</span></div><button className="icon-button mobile-only" aria-label="关闭菜单" onClick={() => setOpen(false)}><X /></button></div><nav aria-label="主导航">{links.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => isActive ? 'active' : ''}><Icon size={19} />{label}</NavLink>)}</nav><div className="sidebar__user"><button className="avatar avatar-button" aria-label="查看我的资料" onClick={() => setProfileOpen(true)}>{user.displayName.slice(0, 1)}</button><button className="sidebar__identity" onClick={() => setProfileOpen(true)}><strong>{user.displayName}</strong><span>{roleNames[user.role]} · 我的资料</span></button><button className="icon-button" onClick={() => void logout()} aria-label="退出登录"><LogOut size={18} /></button></div></aside>
