...
export function AuthPage() {
  ...
  return <main className="auth-page">
    <section className="auth-story" aria-label="产品介绍">
      <div className="brand"><span className="brand__mark"><CalendarDays /></span><span>前程π日程</span></div>
    ...
    <section className="auth-form-wrap">
      <form className="auth-form" onSubmit={submit}>
        <div><p className="eyebrow">{mode === 'login' ? '欢迎回来' : '开始使用'}</p><h2>{mode === 'login' ? '登录前程π日程' : '创建新账号'}</h2><p className="muted">使用姓名作为登录账号</p></div>
        <Notice error={error} />
