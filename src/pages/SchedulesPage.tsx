...
export function SchedulesPage() {
  ...
  async function exportRows() {
    ...
    try {
      ...
      rows.forEach((row) => sheet.addRow({ date: row.class_date, time: `${row.start_time}-${row.end_time}`, subject: safeCell(row.subject), teacher: safeCell(row.teacher_name), student: safeCell(row.student_names.join('、')), room: safeCell(row.classroom), hours: row.lesson_hundredths / 100, status: row.is_completed ? '已完课' : '未完课' }));
