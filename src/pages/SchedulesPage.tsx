...
export function SchedulesPage() {
  ...
  async function exportRows() {
    ...
    try {
      ...
      rows.forEach((row) => sheet.addRow({ date: row.class_date, time: `${row.start_time}-${row.end_time}`, subject: safeCell(row.subject), teacher: safeCell(teacherNameForViewer(row.teacher_name, user?.role)), student: safeCell(row.student_names.join('、')), room: safeCell(row.classroom), hours: row.lesson_hundredths / 100, status: row.is_completed ? '已完课' : '未完课' }));
...
function ScheduleRow({ item, admin, canEdit, canComplete, selected, onSelect, onEdit, onDelete, onComplete }: { item: Schedule; admin: boolean; canEdit: boolean; canComplete: boolean; selected: boolean; onSelect: (value: boolean) => void; onEdit: () => void; onDelete: () => void; onComplete: () => void }) {
  const { user } = useAuth();
  return <tr>{admin && <td><input type="checkbox" aria-label={`选择 ${item.subject}`} checked={selected} onChange={(e) => onSelect(e.target.checked)} /></td>}<td><strong>{item.class_date}</strong><span>{item.start_time}–{item.end_time}</span></td><td>{item.subject}</td><td>{teacherNameForViewer(item.teacher_name, user?.role)}</td><td title={item.student_names.join('、')}>{compactStudentNames(item)}</td><td>{item.classroom || '—'}</td><td>{lessonText(item.lesson_hundredths)}</td><td><span className={`status ${item.is_completed ? 'status--done' : ''}`}>{item.is_completed ? '已完课' : '未完课'}</span></td><td><div className="row-actions">{canComplete && <button aria-label={item.is_completed ? '取消完课' : '标记完课'} title={item.is_completed ? '取消完课' : '标记完课'} onClick={onComplete}>{item.is_completed ? <X /> : <Check />}</button>}{canEdit && <><button aria-label="编辑" onClick={onEdit}><Pencil /></button><button aria-label="删除" onClick={onDelete}><Trash2 /></button></>}{!canEdit && <button className="text-button" onClick={onEdit}>查看</button>}</div></td></tr>;
}

function ScheduleMobileCard({ item, canEdit, canComplete, onEdit, onDelete, onComplete }: { item: Schedule; canEdit: boolean; canComplete: boolean; onEdit: () => void; onDelete: () => void; onComplete: () => void }) {
  const { user } = useAuth();
  return <article className="mobile-card"><header><div><strong>{item.subject}</strong><span>{item.class_date} · {item.start_time}–{item.end_time}</span></div><span className={`status ${item.is_completed ? 'status--done' : ''}`}>{item.is_completed ? '已完课' : '未完课'}</span></header><dl><div><dt>教师</dt><dd>{teacherNameForViewer(item.teacher_name, user?.role)}</dd></div><div><dt>学生</dt><dd>{compactStudentNames(item)}</dd></div><div><dt>教室</dt><dd>{item.classroom || '—'}</dd></div><div><dt>课时</dt><dd>{lessonText(item.lesson_hundredths)}</dd></div></dl><footer>{canComplete && <Button variant="ghost" onClick={onComplete}>{item.is_completed ? '取消完课' : '标记完课'}</Button>}<Button variant="secondary" onClick={onEdit}>{canEdit ? '编辑' : '查看'}</Button>{canEdit && <Button variant="danger" onClick={onDelete}>删除</Button>}</footer></article>;
}
