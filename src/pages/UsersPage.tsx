...
import { ChevronDown, KeyRound, Pencil, Plus, PlusCircle, Trash2, UserCheck, UserX } from 'lucide-react';
...
export function UsersPage() {
  ...
  return <div className="page"><header className="page-header"><div><p className="eyebrow">账号与课时</p><h1>用户管理</h1><p>维护教师、学生和管理员账号。停用不会影响历史排课。</p></div><Button onClick={() => setEditing('new')}><Plus size={18} />新增用户</Button></header><Notice error={error} />
    ...
    <AdjustmentDialog item={adjusting && { id: adjusting.id, name: adjusting.display_name, remainingHundredths: adjusting.remaining_hundredths ?? 0 }} onClose={() => setAdjusting(null)} onSaved={() => { setAdjusting(null); void load(); }} />
